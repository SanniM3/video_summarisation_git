...
class TransNetV2:
    ...
    @staticmethod
    def predictions_to_scenes(predictions: np.ndarray, threshold: float = 0.1):
    ...
    @staticmethod
    def predictions_to_N_frames(video: np.ndarray, predictions: np.ndarray, N=6):
        
        frame_numbers = np.zeros(6)

        # find all transition probabilities over 0.5
        top_prob_frame_numbers = np.argwhere(predictions >= 0.5)

        # uniformly sample if no transitions
        if len(top_prob_frame_numbers) == 0:
            frame_numbers = np.linspace(0, len(video)-1, num=N, dtype=np.uint8)
            num_scenes = 1

        # otherwise sample one frame from each scene, 
        # keep going until 6 are chosen
        else:
            # create scenes
            predictions[:] = 0
            predictions[top_prob_frame_numbers] = 1
            scenes = []
            t, t_prev, start = -1, 0, 0
            for i, t in enumerate(predictions):
                if t_prev == 1 and t == 0:
